...
class FacebookGroupScraper:
    ...
    def generate_post_id(self, post_url: str, text: str) -> str:
        """Generate a deterministic, unique ID for a Facebook post."""
        if post_url:
        ...
        # Hash text to generate signature ID
        import hashlib
        clean_text = "".join(text.split())
        text_hash = hashlib.md5(clean_text.encode('utf-8')).hexdigest()
        return f"fb_hash_{text_hash[:16]}"

    def load_group_registry(self) -> list:
        """Load group list from results/groups.json."""
        filepath = 'results/groups.json'
        if os.path.exists(filepath):
            try:
                ...
                print(f"⚠️ Failed to load group registry: {e}")
        return []

    def save_group_registry(self, groups: list):
        """Save group list to results/groups.json."""
        ...
        try:
            ...
            print(f"⚠️ Failed to save group registry: {e}")

    def choose_target_group_interactively(self) -> str:
        """Present menu of configured Facebook groups, ask user to select or add new."""
        groups = self.load_group_registry()
        
        # Fallback default group if registry is empty
        env_default = os.getenv('FACEBOOK_TARGET_GROUP', 'https://www.facebook.com/groups/flatsandflatmatesbaner')
        if env_default and env_default not in groups:
            groups.append(env_default)
            self.save_group_registry(groups)

        print("\n👥 Configured Facebook Groups:")
        for idx, g in enumerate(groups):
            print(f"{idx + 1}. {self.get_safe_group_dir_name(g)} ({g})")
        print(f"{len(groups) + 1}. ➕ Add a new group URL/name")

        choice = ""
        while not choice.isdigit() or int(choice) < 1 or int(choice) > len(groups) + 1:
            choice = input(f"Enter choice (1-{len(groups) + 1}): ").strip()

        choice_idx = int(choice) - 1
        if choice_idx == len(groups):
            new_target = input("Enter new Facebook group URL or name: ").strip()
            if not new_target:
                print("❌ Invalid target. Using default.")
                return env_default
            if new_target not in groups:
                groups.append(new_target)
                self.save_group_registry(groups)
            return new_target
        else:
            return groups[choice_idx]

    def acquire_group_lock(self, group_url_or_name: str) -> bool:
        """Acquire process lock for target group dir. Returns True if successful."""
        safe_name = self.get_safe_group_dir_name(group_url_or_name)
        target_dir = os.path.join('results', safe_name)
        os.makedirs(target_dir, exist_ok=True)
        lock_file = os.path.join(target_dir, 'scraper.lock')
        ...
        if os.path.exists(lock_file):
            try:
                ...
                if pid_str.isdigit():
                    pid = int(pid_str)
                    # Check if process is still active (signal 0 does not kill process but checks if it exists)
                    os.kill(pid, 0)
                    print(f"❌ Error: Another instance of the scraper (PID {pid}) is already running on this group: '{safe_name}'.")
                    print("Exiting to prevent conflicts.")
                    return False
            except OSError:
                # PID is dead or process does not exist
                print(f"⚠️ Warning: Found stale lock file from crashed process (PID {pid_str}). Removing lock and continuing...")
            except Exception as e:
                print(f"⚠️ Lock check error: {e}. Attempting cleanup...")
    ...
    def release_group_lock(self, group_url_or_name: str):
        """Release lock file for target group dir."""
        safe_name = self.get_safe_group_dir_name(group_url_or_name)
        lock_file = os.path.join('results', safe_name, 'scraper.lock')
        if os.path.exists(lock_file):
            try:
                os.remove(lock_file)
            except Exception as e:
                print(f"⚠️ Failed to remove lock file {lock_file}: {e}")
    ...
    def process_group_posts(self, group_url_or_name, max_posts=50, scrape_only=False):
        ...
        raw_posts_dict = {p.get('message_id'): p for p in existing_raw_posts if p.get('message_id')}
                ...
                try:
                    with open(raw_file, 'w', newline='', encoding='utf-8') as rf:
                        ...
                        for idx_in_dom, post, post_url in target_elements:
                            ...
                            message_id = self.generate_post_id(data.get('post_url'), text)
                            data['message_id'] = message_id

                            # Check duplicate across runs and within current run
                            if message_id in raw_posts_dict or message_id in processed_ids or message_id in processed_in_run:
                                print(f"⏭️ Skipping already processed post: {message_id}")
                                continue

                            html_path = os.path.join(self.group_dir, 'html', f"{message_id}.html")
                            self.save_post_html(post, html_path)
                            data['html_file'] = html_path

                            raw_data_list.append(data)
                            writer.writerow([message_id, text, str(data.get('timestamp') or datetime.now()), data.get('post_url', '')])
                    ...
                    for data in raw_data_list:
                        ...
                        processed_in_run.add(message_id)

                        if not text:
                            log_entry = (
                                f"=== Post ID: {message_id} (Scraper Mode) ===\n"
                        ...
                        if scrape_only:
                            ...
                            raw_posts_dict[message_id] = raw_entry
                            print(f"📝 Scraped raw post: {message_id} - URL: {post_url or 'N/A'}")
                            continue
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Facebook Group House Hunting Bot")
    parser.add_argument('--group', type=str, help="Target Facebook group URL or name (bypasses menus)")
    parser.add_argument('--posts', type=int, help="Max posts to scrape (bypasses .env config)")
    parser.add_argument('--mode', type=str, choices=['1', '2', '3'], help="Mode: 1 (Scrape only), 2 (Offline AI analysis), 3 (Scrape & Analyze)")
    ...
    if choice == '2':
        # Offline AI analysis does not require browser scraping or locks
        scraper.analyze_scraped_posts()
        return

    # Determine targeted group target
    group_target = args.group
    if not group_target:
        if args.mode:  # If mode was passed but not group, fallback to default or prompt
            group_target = os.getenv('FACEBOOK_TARGET_GROUP')
            if not group_target:
                print("❌ Error: No group specified via --group or FACEBOOK_TARGET_GROUP env.")
                return
        else:
            # Fall back to interactive selector menu
            group_target = scraper.choose_target_group_interactively()
    ...
    # Process group lock to avoid concurrent runs on the same group directory
    if not scraper.acquire_group_lock(group_target):
        return
    ...
    try:
        ...
        api_success = False
        if api_token and group_id and not args.group: # only default to Graph API if no custom group target passed
        ...
        scraper.release_group_lock(group_target)
